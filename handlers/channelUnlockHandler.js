...
export default async (ctx) => {
  ...
  const answerQuery = async (text = '', showAlert = false) => {
    if (!answered) {
      try {
        ...
        console.log('[channelUnlockHandler] Error answering callback query:', error.message);
      }
    }
  };
  ...
  } else if (callbackData && callbackData.startsWith('channel_unlock_')) {
    const idString = callbackData.replace('channel_unlock_', '');
  ...
  try {
    ...
    await updateChannelLockStatus(channelID, false);

    await answerQuery('قفل کانال باز شد', false);
    ...
    try {
      ...
      console.log('[channelUnlockHandler] Could not get real member count:', error.message);
    }
    ...
    const lockStatus = '🔓 باز';
    const lockButtonText = '🔒 قفل کردن';
    const lockCallbackData = `channel_lock_${channelID}`;
...
<b>لیبل دکمه:</b> ${updatedChannel.buttonLabel || 'تایید عضویت'}`;
    ...
    try {
      ...
      if (error.description && error.description.includes('message is not modified')) {
        console.log('[channelUnlockHandler] Message not modified');
      } else {
        console.error('[channelUnlockHandler] Error editing message:', error);
    ...
    console.error('[channelUnlockHandler] Error:', error);
