...
export default async (ctx) => {
  ...
  const match = ctx.callbackQuery?.data?.match(/^plan_order_reject_(\d+)$/);
  if (!match) return;
  ...
  await updatePlanOrder(orderId, { status: 'rejected', rejectedBy: userID });
  ...
  const doneMarkup = {
    inline_keyboard: [[{ text: '❌ رد شده', callback_data: 'plan_order_rejected' }]]
  };
  ...
  try {
    await ctx.telegram.sendMessage(
      order.userID,
      '❌ درخواست خرید اشتراک شما رد شد. در صورت واریز مبلغ با پشتیبانی تماس بگیرید.',
      { reply_markup: { inline_keyboard: [[{ text: '🔙 بازگشت به منو', callback_data: 'back_to_main' }]] } }
    );
  } catch (_) {}
  await ctx.answerCbQuery({ text: 'سفارش رد شد' });
};
