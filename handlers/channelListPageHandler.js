...
export default async (ctx) => {
  ...
  if (!callbackData || !callbackData.startsWith('channel_list_page_')) {
    return;
  }

  const page = parseInt(callbackData.split('_')[3], 10);
  ...
  try {
    ...
    if (channels.length === 0) {
      ...
      try {
        ...
        if (error.description && error.description.includes('message is not modified')) {
          console.log('[channelListPageHandler] Message not modified');
        } else {
          console.error('[channelListPageHandler] Error editing message:', error);
    ...
    message += `<b>صفحه:</b> ${validPage} از ${totalPages}\n\n`;
    ...
    if (totalPages > 1) {
      const paginationRow = [];
      if (validPage > 1) {
        paginationRow.push({ text: '⬅️ قبلی', callback_data: `channel_list_page_${validPage - 1}` });
      }
      if (validPage < totalPages) {
        paginationRow.push({ text: '➡️ بعدی', callback_data: `channel_list_page_${validPage + 1}` });
      }
    ...
    console.error('[channelListPageHandler] Error:', error);
