...
/**
 * بروزرسانی آمار و نمایش مجدد جزئیات سرور (همراه با بررسی اتصال و لاگ کنسول)
 */
export default async (ctx) => {
  ...
  if (ctx.match && Array.isArray(ctx.match) && ctx.match[1]) {
    ...
    if (data && data.startsWith('server_refresh_')) {
      serverId = parseInt(data.replace('server_refresh_', ''), 10);
    }
  }
  ...
  try {
    await ctx.answerCbQuery({ text: '⏳ در حال بروزرسانی آمار...', show_alert: false });
  } catch (_) {}

  try {
    const server = await findServerByDatabaseID(serverId);
    if (!server) {
      console.log(`[serverRefreshHandler] Server not found: id=${serverId}`);
      await ctx.answerCbQuery({ text: 'سرور یافت نشد', show_alert: true }).catch(() => {});
      return;
    }

    console.log(`[serverRefreshHandler] Refreshing server stats: id=${serverId}, name=${server.serverName}`);
    ...
    console.error('[serverRefreshHandler] Error:', error);
    try {
      await ctx.answerCbQuery({ text: 'خطا در بروزرسانی آمار', show_alert: true });
    } catch (_) {}
  }
};
